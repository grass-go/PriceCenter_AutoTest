...
public class AllAssetList extends TronlinkBase {
  ...
  public void allAssetList02() {
    ...
    Object actualBT =
        JSONPath.eval(responseContent, "$..data.token[name='BitTorrent Old'].balanceStr");
    JSONArray actualBTArray = (JSONArray) actualBT;
    Assert.assertEquals("9", actualBTArray.get(0));
    Object actualBTPrice =
        JSONPath.eval(responseContent, "$..data.token[name='BitTorrent Old'].price");
    ...
    Object national =
        JSONPath.eval(responseContent, "$..data.token[name='BitTorrent Old'].national[0]");
    Assert.assertEquals("", national.toString());

    // check WINkLink balance
    Object actualWL =
        JSONPath.eval(
            responseContent, String.join("", "$..data.token[name='WINkLink'].balanceStr"));
    ...
    national =
        JSONPath.eval(responseContent, "$..data.token[name='Decentralized USD'].national[0]");
    Assert.assertEquals("DM", national.toString());
  ...
  public void allAssetList03() {
    ...
    for (Map.Entry<String, String> entry : jTokens.entrySet()) {
      ...
      Object Price =
          JSONPath.eval(
              responseContent, String.join("", "$..data.token[shortName='", shortName, "'].price"));
      ...
      for (index = 0; index < 5; index++) {
        ...
        String reqestUrl =
            TronlinkBase.tronscanApiUrl
                + "/api/token_trc20?contract="
                + contractAddr
                + "&showAll=1";
        ...
        if (total == 0) {
          ...
          log.info(
              String.join(
                  "", "$..trc20_tokens[contract_address='", contractAddr, "'].contract_name"));
          Object expPrice =
              JSONPath.eval(
                  transcanRspContent,
                  String.join(
                      "",
                      "$..trc20_tokens[contract_address='",
                      contractAddr,
                      "'].market_info.priceInTrx"));
  ...
  public void allAssetList04() throws InterruptedException {
    ...
    for (int i = 0; i < token10sArray.size(); i++) {
      if (!token10sArray.get(i).equals("")) {
        ...
        Object cur_isOfficial =
            JSONPath.eval(
                responseContent, String.join("", "$..data.token[id='", curId, "'].isOfficial[0]"));
        ...
        Thread.sleep(1000);
        JSONObject transcanRspContent = TronlinkApiList.parseResponse2JsonObject(transcanRsp);
        Object scan_levelObject =
            JSONPath.eval(transcanRspContent, String.join("", "$..data[0].level"));
        ...
        log.info(
            "curId:"
                + curId
                + ", cur_isOfficial:"
                + cur_isOfficial.toString()
                + ", transcan level:"
                + scan_levelObject.toString());
        log.info("======");
    ...
    for (int i = 0; i < token20sArray.size(); i++) {
      if (!token20sArray.get(i).equals("")) {
        ...
        Object cur_isOfficial =
            JSONPath.eval(
                responseContent,
                String.join("", "$..data.token[contractAddress='", curAddress, "'].isOfficial[0]"));
        String requestUrl =
            TronlinkBase.tronscanApiUrl
                + "/api/token_trc20?contract="
                + curAddress
                + "&showAll=1&source=true";
        HttpResponse transcanRsp = TronlinkApiList.createGetConnect(requestUrl, null, null, null);
        Thread.sleep(1000);
        JSONObject transcanRspContent = TronlinkApiList.parseResponse2JsonObject(transcanRsp);
        Object scan_levelObject =
            JSONPath.eval(transcanRspContent, String.join("", "$..trc20_tokens[0].level"));
        ...
        if (curAddress.equals("TAFjULxiVgT4qWk6UZwjqwZXTSaGaqnVp4")
            || curAddress.equals("TPYmHEhy5n8TCEfYGqW2rPxsghSfzghPDn")) {
          Assert.assertEquals(Math.abs(Integer.parseInt(cur_isOfficial.toString())), 1);
        } else {
          Assert.assertEquals(
              Math.abs(Integer.parseInt(cur_isOfficial.toString())), scan_level + 1);
        }
      }
    }
  }

  @Test(enabled = true, description = "Test totalBalanceStr ")
  public void allAssetList05() {
    params.clear();
    params.put("address", quince_B58);
    response = TronlinkApiList.V2AllAssetList(params);
    Assert.assertEquals(response.getStatusLine().getStatusCode(), 200);
    ...
    for (int i = 0; i < jSONArray.size(); i++) {
      ...
      String tmp = jSONObject.getString("totalBalance");
      String totalBalance = new BigDecimal(tmp).toPlainString();
      log.info("totalBalance:" + totalBalance);
      if (!jSONObject.getString("totalBalanceStr").equals(totalBalance)) {
