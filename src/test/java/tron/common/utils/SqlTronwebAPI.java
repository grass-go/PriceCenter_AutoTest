...
public class SqlTronwebAPI implements IReporter{
...
//            output.close();
  ...
  public void mysql(){
    HttpResponse response = TronscanApiList.getBuildId("http://tronlink:tronlink@192.168.20.208:8080/job/Tronweb_Monitor/api/json");
    ...
    try{
      ...
      String sql = "INSERT INTO `AutoTestScan`.`tronwebAPI`(`time`, `status`, `sucessclass`, `sucessnum`,`failClass`,`failnum`,`sum`,`buildid`) VALUES ('"+time+"','"+status+"','"+sucessClass+"','"+sucessnum+"','"+failClass+"','"+failnum+"','"+sum+"','"+buildid+"')";
